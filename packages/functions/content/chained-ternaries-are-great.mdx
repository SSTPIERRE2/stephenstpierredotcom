...
<h2 id="chained-vs-nested-logic">Chained vs Nested Logic</h2>
...
<img
  src="./content/images/picard_maneuver.webp"
  alt="Captain Jean-Luc Picard from Star Trek: The Next Generation tugging his shirt."
/>
...
The `if` version has extra noise including the `if` keyword vs `?` in the ternary version, `return` which is implicit in the ternary, extra braces, and semicolons. To reason about why less code is good in this case, let's first think about the job of a ternary vs an if statement.

Ternaries evaluate to a value, while if statements are more powerful in that they can and frequently do cause side effects or mutate variables. Since we don't need to do either of those, the ternary option seems like the right tool for the job instead of forcing `if` to do this simple task.

That said, I generally prefer to use `if` whenever nesting, mutations, or side effects are unavoidable.
...
I think a big reason why ternaries have been a controversial topic in the JavaScript community is formatting. Before [Prettier](https://prettier.io/) existed, our options for consistently formatting code were somewhat limited, for example my first team used eslint with manual formatting.

Eslint actually had some rules about ternaries, and I think the [no-nested-ternary](https://eslint.org/docs/latest/rules/no-nested-ternary) rule sparked a lot of confusion. I distinctly remember presenting a case to my team for why we should turn the rule off and how we should format them going forward based on Eric's post.
...
The official MDN documentation which I generally consider to have to final word on core web development topics calls this a [conditional chain](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Conditional_operator#conditional_chains).

<h2 id="prettiers-quest-for-an-acceptable-format">
  Prettier's Quest for an Acceptable Format
</h2>
...
Eventually they developed a novel solution drawing inspiration from the original desire for indentation as well as other languages like Rust and OCaml, I actually just started using the `experimental-ternaries` option on this blog!
...
I still enjoy the form I learned from Eric for the same reason why I avoid using `if` without any curly braces and `return` on the same line, but this new form where each conditional is grouped with its return value is growing on me in the case of ternaries.
...
Ternaries are simply `if..else` statements that produce a value and have been around since the language's early days, so we shouldn't be afraid to use them and even chain them together in most simple cases.
...
It's a perfectly valid way to produce a value based on some logic, which I believe even has some advantages over `if` statements.
