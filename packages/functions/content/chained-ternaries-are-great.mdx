...
Eslint even includes a [no-ternary](https://eslint.org/docs/latest/rules/no-ternary) rule disallowing ternaries altogether, that seems a little extreme! What's next, enable disallowing optional chaining for folks who prefer explicit checks with `if`?

<h2 id="prettiers-quest-for-a-solution">Prettier's Quest for a Solution</h2>
