...
It seemed rather unconventional, and I'm still not sure if this is how grid is intended to be used, but it's still pretty cool! Let's work through what this technique can do for us with an interactive demo.
...
You see, once we start using more grid columns as dynamic gaps, using the <InlineCode>gap-column-gap</InlineCode> property itself becomes redundant since we're inserting it manually. We're essentially making our whole layout, including gaps, a function of the available space.
...
Essentially, the minimum content size of the tallest <InlineCode>1fr</InlineCode> row becomes the value of <InlineCode>1fr</InlineCode>, so since we set each row to <InlineCode>1fr</InlineCode>, the height of each becomes the height of the second section which is the tallest.
...
Here we're using the default grid flow where it uses a single column, but we're setting three explicit rows with <InlineCode>grid-template-rows: min-content 1fr min-content</InlineCode>.
...
I hope this post encourages more developers to experiment and make more, wild grid recipes. For further reading, check out Josh W. Comeau's [interactive guide to CSS Grid](https://www.joshwcomeau.com/css/interactive-guide-to-grid/#grid-flow-2) which providers a more comprehensive guide on how it all works.
